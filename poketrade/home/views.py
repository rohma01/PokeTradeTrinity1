...
def marketplace_view(request):
    ...
    if search_query:
        listings = listings.filter(pokemon__name__icontains=search_query)
    ...
    for listing in listings:
        ...
        if response.status_code == 200:
            ...
            types = []
            stats = {}
        desired_list = listing.desired_pokemon_list() if hasattr(listing, 'desired_pokemon_list') else []
        has_desired = bool(set(user_pokemon_names) & set(desired_list)) if desired_list else False
        pokemon_data.append({
